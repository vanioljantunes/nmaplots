#' nmaplots: Publication-quality network plots for netmeta and gemtc objects
#'
#' Draws the evidence network of a network meta-analysis fitted with
#' \pkg{netmeta} or \pkg{gemtc}. One function, [nmaplot()], takes the same
...
#' treatment name and the event rate as fractions (events/n) at every node, share of patients inside the disc, an optional outer ring per
...
#' number comes from the fitted object.
#'
#' @section Guides (read in order):
#' \enumerate{
#'   \item [nmaplots-1-workflow] -- from a `netmeta` or `gemtc` object to a saved figure.
